...
for x in xrange(0, dimension):
    for y in xrange(0, dimension - 3):
        product = grid[x][y] * grid[x][y + 1] * grid[x][y + 2] * grid[x][y + 3]
        if product > max_product:
            max_product = product

# Calculate Left
for x in xrange(0, dimension - 3):
    for y in xrange(0, dimension):
        product = grid[x][y] * grid[x + 1][y] * grid[x + 2][y] * grid[x + 3][y]
        if product > max_product:
            max_product = product

# Calculate Diagonal Left
for x in xrange(0, dimension - 3):
    for y in xrange(0, dimension - 3):
...
# Calculate Diagonal Right
for x in xrange(3, dimension):
    for y in xrange(0, dimension - 3):
        product = grid[x][y] * grid[x - 1][y + 1] * grid[x - 2][y + 2] * grid[x - 3][y + 3]
        if product > max_product:
            max_product = product
